...
type ElWithPopover = HTMLElement & {
  ...
  _create_popover?: () => void; // Čistý způsob, jak to zavolat z updated
};
    ...
    updated(el: ElWithPopover, binding: DirectiveBinding) {
      ...
      if (el._popover) {
        if (is_empty) {
          ...
          // Původní logika: probudíme tooltip odesláním eventu, pokud je myš nad ním
          if (
            el._popover.style.display === "none" &&
            !click &&
            el.matches(":hover")
        ...
        // Vytvoříme HTML popover (pokud právě přišel text), aby se předrenderoval
